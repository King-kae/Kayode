...
class IconsView {
    constructor(_extensionUri, _storage) {
        ...
        this.onDidReceiveMessage = (data) => {
            switch (data.type) {
                case enum_constants_modal_1.Action.SET_VIEW_STATE: {
                    this._storage.setValue(data.payload.key, data.payload.value);
                    break;
                }
                case enum_constants_modal_1.Action.COPY: {
                    this.copyIcon(data.payload);
                    break;
                }
                case enum_constants_modal_1.Action.SAVE: {
    ...
    async resolveWebviewView(webviewView, _context, _token) {
        ...
        this._icons = await (0, utilities_1.getIcons)();
    ...
    copyIcon({ selectedIcon, copyType }) {
    ...
    async saveIcon(selectedIcon = this._storage.getValue("selectedIcon", this._icons[0])) {
        ...
        if (extension === ".png" && sharp) {
            let iconColor = Settings_1.Settings.pngIconColor;
            if (Settings_1.Settings.pngIconColor === 'Prompt') {
            ...
            const png = await sharp(Buffer.from(selectedIcon.svg))
                .resize({
                width: Settings_1.Settings.pngDimensions.width,
                height: Settings_1.Settings.pngDimensions.height,
                fit: 'inside'
            })
        ...
        // Save as .svg file
        if (extension === ".svg") {
            await vscode.workspace.fs.writeFile(savedPathUri, new util_1.TextEncoder().encode(selectedIcon.svg));
        }
        await vscode.commands.executeCommand("vscode.open", savedPathUri);
    }
    async downloadIconArchive() {
        const savedPathUri = await vscode.window.showSaveDialog({
        ...
        if (savedPathUri) {
            await vscode.workspace.fs.writeFile(savedPathUri, new util_1.TextEncoder().encode(JSON.stringify(this._icons, null, vscode.window.activeTextEditor?.options.tabSize || 2)));
            await vscode.commands.executeCommand("vscode.open", savedPathUri);
        }
    }
    async refreshView() {
    ...
    _getHtmlForWebview(webview) {
        ...
        const { showIconName, showIconInfo, showCategoryBadge, iconFamily, iconCategory, iconSize, zoom, searchText, viewType } = this.getViewState();
        const families = [...new Set(this._icons.map(icon => icon.family.toLowerCase()).filter(Boolean))];
        const categories = [...new Set(this._icons.map(icon => icon.categories).flat().map(category => category.toLowerCase()).filter(Boolean))];
...
exports.IconsView = IconsView;
IconsView.viewType = enum_constants_modal_1.ICONS_VIEW;
function getNonce() {
    let text = '';
...
//# sourceMappingURL=IconsView.js.map